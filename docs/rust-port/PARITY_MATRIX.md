...
| Step lifecycle state invariants | IMPLEMENTED, CI PENDING | The TypeScript characterization suite proves `StepSchema` accepts contradictory states. Rust `StepState` rejects cross-state fields, null-smuggled forbidden fields, missing modern terminal payloads, and payload reflection in errors while preserving valid and legacy spec-version 1 projections. Full step request/response contracts, date coercion, serialized payload decoding, and backend adoption remain. |

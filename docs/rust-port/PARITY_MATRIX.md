...
| Core run capability negotiation | IMPLEMENTED, CI PENDING | Production Rust preserves conservative invalid-version fallback and the exact encryption, framing, compression, and sealed-format semantic-version cutoffs. Direct and TypeScript/Rust differential tests are committed under WF-RUST-097. Runtime producer adoption remains. |

...
fn rejects_invalid_duration_strings() {
    assert!(parse_duration_to_unix_ms(DurationInput::String("invalid".to_owned()), NOW).is_err());
}
...
fn rejects_date_like_values_with_non_finite_timestamps() {
    let error =
        parse_duration_to_unix_ms(DurationInput::DateMilliseconds(f64::INFINITY), NOW).unwrap_err();
    assert!(
        error
            .message()
            .contains("Expected a valid Date with a finite timestamp")
...
fn rejects_finite_durations_whose_result_overflows_timeclip() {
    let error = parse_duration_to_unix_ms(DurationInput::Milliseconds(f64::MAX), NOW).unwrap_err();
    assert!(
        error
            .message()
